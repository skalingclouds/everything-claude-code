...
- ECC-Tools commit `632e059e51b6e1297ba118807c8b5b2adbac74ce`
  ...
  confirmed the ECC vault Cloudflare credential works as bearer API auth for
  the ECC Tools account/namespace, while local Wrangler OAuth currently fails
  with Cloudflare authentication error `10000`. Native-payments copy remains
  blocked by Linear ITO-61 until the target Marketplace Pro account has webhook
  provenance and this target readback plus the official internal announcement
  gate pass.
